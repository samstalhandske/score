...
bool save_apparent_temperature_to_report(Open_Meteo_Report_Hourly *report, const SCore_JSON_Object *parent_to_temperature_object, const SCore_JSON_Object *units_object, const char *name) {
...

    {
        ...
        assert(unit == Open_Meteo_Unit_Celsius || unit == Open_Meteo_Unit_Fahrenheit);
...
bool save_rain_to_report(Open_Meteo_Report_Hourly *report, const SCore_JSON_Object *parent_to_object, const SCore_JSON_Object *units_object, const char *name) {
...

    {
        ...
        assert(open_meteo_string_to_unit(unit_text, &unit));
        assert(unit == Open_Meteo_Unit_Millimeter);
...
void open_meteo_write_url_to_writer(SCore_Buffer_Writer *writer, const Open_Meteo_Data_Request *data_request) {
    assert(
        data_request->temperature_unit == Open_Meteo_Unit_Celsius ||
        data_request->temperature_unit == Open_Meteo_Unit_Fahrenheit
    );
...
SCORE_BOOL open_meteo_string_to_unit(const char *str, Open_Meteo_Unit *unit) {
    ...
    if(score_string_compare(str, "°C", false) == SCore_String_Compare_Result_Equal) {
        *unit = Open_Meteo_Unit_Celsius;
    }
    else if(score_string_compare(str, "°F", false) == SCore_String_Compare_Result_Equal) {
        *unit = Open_Meteo_Unit_Fahrenheit;
    }
