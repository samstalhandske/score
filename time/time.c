...
SCORE_BOOL score_time_from_json(const SCore_JSON_Object *json_object, SCore_Time *out_time) {
    if(score_json_is_string(json_object)) { /* ISO 8601. */
...

        {
            ...
            assert(score_time_set_iso_8601(
                out_time,
    ...
    else if(score_json_is_number(json_object)) { /* Unix. */
        double v = 0;
        assert(score_json_as_number(json_object, &v));
        assert(score_time_set_unix(out_time, (uint32_t)v, 0));
    }
    else {
        return SCORE_FALSE;
    }
